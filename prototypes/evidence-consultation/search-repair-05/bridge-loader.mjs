...
import { adapt } from "./checked-adapter.mjs";
...
export async function resolve(specifier,context,nextResolve) {
  const url = specifier.startsWith("file:") ? new URL(specifier) : specifier.startsWith(".") && context.parentURL ? new URL(specifier,context.parentURL) : null;
  if (url?.protocol==="file:" && path.dirname(fileURLToPath(url))===directory && virtual.has(path.basename(fileURLToPath(url)))) return {url:url.href,shortCircuit:true};
